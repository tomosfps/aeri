...
export const interaction: PaginatedMessageContextCommand<AnimeUserScoreItem> = {
    ...
    async getItems(interaction) {
        ...
        if (nsfwMediaCount > 0 && !interaction.isNSFW && filteredRelations.length === 0) {
            container.updateComponent(
                "warning",
                `NSFW media was filtered out and no other media was found close to ${inlineCode(anime)}\nTo view them, use this command in a NSFW channel.`,
            );
            await interaction.followUpContainer(true);
            return undefined;
        }
        ...
        if (filteredRelations.length === 0) {
            container.updateComponent("warning", `Could not find a relation close to ${inlineCode(anime)}`);
            await interaction.followUpContainer(true);
            return undefined;
        }
    ...
    async execute(interaction) {
        ...
        try {
            await createSimplePagination(this, interaction, "message-context-anime");
        } catch (error: any) {
            logger.error("Error in anime message-context command", "AnimeMessageContext", { error });
